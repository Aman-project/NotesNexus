...
import { Filter, Tag, SortAsc, SortDesc, X, Check } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { memo } from "react";

const MobileFilterDialog = memo(({
  isOpen,
  setIsOpen,
  selectedCategory,
  setSelectedCategory,
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="bg-background/95 backdrop-blur-md border-border rounded-xl max-w-sm mx-auto shadow-lg">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-600 dark:from-primary dark:to-blue-400">
            Filter Notes
          </DialogTitle>
          <DialogDescription className="text-center text-muted-foreground">
            Customize your view with the options below
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <motion.div className="space-y-3" variants={itemVariants}>
            ...
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              <SelectTrigger className="w-full border-border bg-background/80 focus:ring-primary/20 rounded-lg">
            ...
            {selectedCategory !== "all" && (
              <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 px-3 py-1 mt-2 rounded-full">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-3 pt-2">
          ...
          <Button 
            className="w-full bg-gradient-to-r from-primary to-blue-600 hover:opacity-90 transition-all rounded-full flex items-center justify-center" 
            onClick={() => {
              applyFilters();
              setIsOpen(false);
            }}
